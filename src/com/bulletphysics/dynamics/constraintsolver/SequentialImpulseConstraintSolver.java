...
public class SequentialImpulseConstraintSolver extends ConstraintSolver {
	...
	private final IntArray orderTmpConstraintPool = new IntArray();
	private final IntArray orderFrictionConstraintPool = new IntArray();
...
// }
	...
	public int randInt2 (int n) {
		...
		/*
		 * return new Random(). // seems good; xor-fold and modulus long un = n; long r = rand2();
		 * 
		 * // note: probably more aggressive than it needs to be -- might be // able to get away without one or two of the innermost
		 * branches. if (un <= 0x00010000L) { r ^= (r >>> 16); if (un <= 0x00000100L) { r ^= (r >>> 8); if (un <= 0x00000010L) { r
		 * ^= (r >>> 4); if (un <= 0x00000004L) { r ^= (r >>> 2); if (un <= 0x00000002L) { r ^= (r >>> 1); } } } } }
		 * 
		 * // TODO: check modulo C vs Java mismatch return (int) Math.abs(r % un);
		 */
	}
	...
	private void resolveSplitPenetrationImpulseCacheFriendly (SolverBody body1, SolverBody body2,
		...
		if (contactConstraint.penetration < solverInfo.splitImpulsePenetrationThreshold) {
			...
			tmp.set(contactConstraint.contactNormal).scl(body1.invMass);
			body1.internalApplyPushImpulse(tmp, contactConstraint.angularComponentA, normalImpulse);

			tmp.set(contactConstraint.contactNormal).scl(body2.invMass);
	...
	@StaticAlloc
	protected void addFrictionConstraint (Vector3 normalAxis, int solverBodyIdA, int solverBodyIdB, int frictionIndex,
		ManifoldPoint cp, Vector3 rel_pos1, Vector3 rel_pos2, CollisionObject colObj0, CollisionObject colObj1, float relaxation) {
		...
		Stack stack = Stack.enter();
		Vector3 ftorqueAxis1 = stack.allocVector3();
		Matrix3 tmpMat = stack.allocMatrix3();

		{
			ftorqueAxis1.set(rel_pos1).crs(solverConstraint.contactNormal);
			solverConstraint.relpos1CrossNormal.set(ftorqueAxis1);
			if (body0 != null) {
				solverConstraint.angularComponentA.set(ftorqueAxis1);
				solverConstraint.angularComponentA.mul(body0.getInvInertiaTensorWorld(tmpMat));
			} else {
				solverConstraint.angularComponentA.set(0f, 0f, 0f);
			}
		}
		{
			ftorqueAxis1.set(rel_pos2).crs(solverConstraint.contactNormal);
			solverConstraint.relpos2CrossNormal.set(ftorqueAxis1);
			if (body1 != null) {
				solverConstraint.angularComponentB.set(ftorqueAxis1);
				solverConstraint.angularComponentB.mul(body1.getInvInertiaTensorWorld(tmpMat));
			} else {
		...
		float denom1 = 0f;
		if (body0 != null) {
			vec.set(solverConstraint.angularComponentA).crs(rel_pos1);
			denom0 = body0.getInvMass() + normalAxis.dot(vec);
		}
		if (body1 != null) {
			vec.set(solverConstraint.angularComponentB).crs(rel_pos2);
	...
	public float solveGroupCacheFriendlySetup (ObjectArrayList<CollisionObject> bodies, int numBodies,
		...
		try {
			...
			// if (1)
			{
				...
				/*
				 * { for (int i=0;i<numBodies;i++) { btRigidBody* rb = btRigidBody::upcast(bodies[i]); if (rb && (rb->getIslandTag()
				 * >= 0)) { btAssert(rb->getCompanionId() < 0); int solverBodyId = m_tmpSolverBodyPool.size(); btSolverBody&
				 * solverBody = m_tmpSolverBodyPool.expand(); initSolverBody(&solverBody,rb); rb->setCompanionId(solverBodyId); } } }
				 */
...

				{
					...
					Vector3 pos2 = stack.allocVector3();
					Vector3 vel = stack.allocVector3();
					Vector3 torqueAxis0 = stack.allocVector3();
					Vector3 torqueAxis1 = stack.allocVector3();
					Vector3 vel1 = stack.allocVector3();
					Vector3 vel2 = stack.allocVector3();
					Vector3 frictionDir1 = stack.allocVector3();
					Vector3 frictionDir2 = stack.allocVector3();
					Vector3 vec = stack.allocVector3();
						...
						for (int j = 0; j < manifold.getNumContacts(); j++) {
							...
							if (cp.getDistance() <= 0f) {
...

								{
									...
									torqueAxis0.set(rel_pos1).crs(cp.normalWorldOnB);

									if (rb0 != null) {
										solverConstraint.angularComponentA.set(torqueAxis0);
										solverConstraint.angularComponentA.mul(rb0.getInvInertiaTensorWorld(tmpMat));
									} else {
										solverConstraint.angularComponentA.set(0f, 0f, 0f);
									}

									torqueAxis1.set(rel_pos2).crs(cp.normalWorldOnB);

									if (rb1 != null) {
										solverConstraint.angularComponentB.set(torqueAxis1);
										solverConstraint.angularComponentB.mul(rb1.getInvInertiaTensorWorld(tmpMat));
									} else {
										solverConstraint.angularComponentB.set(0f, 0f, 0f);
									}
									...
									solverConstraint.relpos1CrossNormal.set(rel_pos1).crs(cp.normalWorldOnB);
									solverConstraint.relpos2CrossNormal.set(rel_pos2).crs(cp.normalWorldOnB);
									...
									if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
										...
										if (rb0 != null) {
											tmp.set(solverConstraint.contactNormal).scl(rb0.getInvMass());
										...
										if (rb1 != null) {
											tmp.set(solverConstraint.contactNormal).scl(rb1.getInvMass());
									...
									if (!cp.lateralFrictionInitialized) {
										cp.lateralFrictionDir1.set(cp.normalWorldOnB).scl(rel_vel);
										cp.lateralFrictionDir1.set(vel).sub(cp.lateralFrictionDir1);

										float lat_rel_vel = cp.lateralFrictionDir1.len2();
										if (lat_rel_vel > BulletGlobals.FLT_EPSILON)// 0.0f)
										{
											cp.lateralFrictionDir1.scl(1f / (float)Math.sqrt(lat_rel_vel));
											...
											cp.lateralFrictionDir2.set(cp.lateralFrictionDir1).crs(cp.normalWorldOnB);
											cp.lateralFrictionDir2.nor(); // ??
...

									{
										...
										if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
											...
											if (rb0 != null) {
												tmp.set(frictionConstraint1.contactNormal).scl(rb0.getInvMass());
												tmpSolverBodyPool.getQuick(solverConstraint.solverBodyIdA).internalApplyImpulse(tmp,
													frictionConstraint1.angularComponentA, frictionConstraint1.appliedImpulse);
											}
											if (rb1 != null) {
												tmp.set(frictionConstraint1.contactNormal).scl(rb1.getInvMass());
												tmpSolverBodyPool.getQuick(solverConstraint.solverBodyIdB).internalApplyImpulse(tmp,
									...
									}
									{
										...
										if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
											...
											if (rb0 != null) {
												tmp.set(frictionConstraint2.contactNormal).scl(rb0.getInvMass());
												tmpSolverBodyPool.getQuick(solverConstraint.solverBodyIdA).internalApplyImpulse(tmp,
													frictionConstraint2.angularComponentA, frictionConstraint2.appliedImpulse);
											}
											if (rb1 != null) {
												tmp.set(frictionConstraint2.contactNormal).scl(rb1.getInvMass());
												tmpSolverBodyPool.getQuick(solverConstraint.solverBodyIdB).internalApplyImpulse(tmp,
	...
	protected void prepareConstraints (PersistentManifold manifoldPtr, ContactSolverInfo info, IDebugDraw debugDrawer) {
		...
		// only necessary to refresh the manifold once (first iteration). The integration is done outside the loop
		{
			...
			Matrix3 tmpMat3 = stack.allocMatrix3();
			...
			Vector3 vel2 = stack.allocVector3();
			Vector3 vel = stack.allocVector3();
			Vector3 totalImpulse = stack.allocVector3();
			Vector3 torqueAxis0 = stack.allocVector3();
			Vector3 torqueAxis1 = stack.allocVector3();
			Vector3 ftorqueAxis0 = stack.allocVector3();
			Vector3 ftorqueAxis1 = stack.allocVector3();
			...
			for (int i = 0; i < numpoints; i++) {
				...
				if (cp.getDistance() <= 0f) {
					...
					rel_pos1.set(pos1).sub(body0.getCenterOfMassPosition(tmpVec));
					rel_pos2.set(pos2).sub(body1.getCenterOfMassPosition(tmpVec));

					// this jacobian entry is re-used for all iterations
					Matrix3 mat1 = body0.getCenterOfMassTransform(stack.allocTransform()).basis;
					mat1.transpose();

					Matrix3 mat2 = body1.getCenterOfMassTransform(stack.allocTransform()).basis;
					mat2.transpose();

					JacobianEntry jac = jacobiansPool.get();
					jac.init(mat1, mat2, rel_pos1, rel_pos2, cp.normalWorldOnB, body0.getInvInertiaDiagLocal(stack.allocVector3()),
						body0.getInvMass(), body1.getInvInertiaDiagLocal(stack.allocVector3()), body1.getInvMass());
					...
					totalImpulse.set(cp.normalWorldOnB).scl(cpd.appliedImpulse);
					...
					// /
					{
						...
						cpd.angularComponentA.mul(body0.getInvInertiaTensorWorld(tmpMat3));

						torqueAxis1.set(rel_pos2).crs(cp.normalWorldOnB);

						cpd.angularComponentB.set(torqueAxis1);
						cpd.angularComponentB.mul(body1.getInvInertiaTensorWorld(tmpMat3));
					}
					{
						ftorqueAxis0.set(rel_pos1).crs(cpd.frictionWorldTangential0);

						cpd.frictionAngularComponent0A.set(ftorqueAxis0);
						cpd.frictionAngularComponent0A.mul(body0.getInvInertiaTensorWorld(tmpMat3));
					}
					{
						ftorqueAxis1.set(rel_pos1).crs(cpd.frictionWorldTangential1);

						cpd.frictionAngularComponent1A.set(ftorqueAxis1);
						cpd.frictionAngularComponent1A.mul(body0.getInvInertiaTensorWorld(tmpMat3));
					}
					{
						ftorqueAxis0.set(rel_pos2).crs(cpd.frictionWorldTangential0);

						cpd.frictionAngularComponent0B.set(ftorqueAxis0);
						cpd.frictionAngularComponent0B.mul(body1.getInvInertiaTensorWorld(tmpMat3));
					}
					{
						ftorqueAxis1.set(rel_pos2).crs(cpd.frictionWorldTangential1);

						cpd.frictionAngularComponent1B.set(ftorqueAxis1);
						cpd.frictionAngularComponent1B.mul(body1.getInvInertiaTensorWorld(tmpMat3));
					}
