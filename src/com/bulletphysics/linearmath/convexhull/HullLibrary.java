...
/** HullLibrary class can create a convex hull from a collection of vertices, using the ComputeHull method. The {@link ShapeHull}
 * class uses this HullLibrary to create a approximate convex mesh given a general (non-polyhedral) convex shape.
 *
 * @author jezek2 */
public class HullLibrary {
	...
	public boolean createConvexHull (HullDesc desc, HullResult result) {
		...
		ObjectArrayList<Vector3> vertexSource = new ObjectArrayList<Vector3>();
		MiscUtil.resize(vertexSource, vcount, Suppliers.NEW_VECTOR3_SUPPLIER);

		Stack stack = Stack.enter();
		Vector3 scale = stack.allocVector3();

		int[] ovcount = new int[1];

		boolean ok = cleanupVertices(desc.vcount, desc.vertices, desc.vertexStride, ovcount, vertexSource, desc.normalEpsilon,
		...
		if (ok) {
			// if ( 1 ) // scale vertices back to their original size.
			{
				for (int i = 0; i < ovcount[0]; i++) {
					Vector3 v = vertexSource.getQuick(i);
			...
			if (ok) {
				...
				ObjectArrayList<Vector3> vertexScratch = new ObjectArrayList<Vector3>();
				MiscUtil.resize(vertexScratch, hr.vcount, Suppliers.NEW_VECTOR3_SUPPLIER);
				...
				if (desc.hasHullFlag(HullFlags.TRIANGLES)) { // if he wants the results as triangle!
					...
					MiscUtil.resize(result.outputVertices, ovcount[0], Suppliers.NEW_VECTOR3_SUPPLIER);
					...
					if (desc.hasHullFlag(HullFlags.REVERSE_ORDER)) {
						IntArray source_ptr = hr.indices;
						int source_idx = 0;

						IntArray dest_ptr = result.indices;
						int dest_idx = 0;
	...
	private boolean computeHull (int vcount, ObjectArrayList<Vector3> vertices, PHullResult result, int vlimit) {
	...
	private int calchullgen (ObjectArrayList<Vector3> verts, int verts_count, int vlimit) {
		...
		// int j;
		Vector3 bmin = stack.alloc((Vector3)verts.getQuick(0));
		Vector3 bmax = stack.alloc((Vector3)verts.getQuick(0));
		IntArray isextreme = new IntArray();
		// isextreme.reserve(verts_count);
		IntArray allow = new IntArray();
		...
		tmp.set(bmax).sub(bmin);
		float epsilon = tmp.len() * 0.001f;
		assert (epsilon != 0f);
		...
		VectorUtil.add(center, verts.getQuick(p.getCoord(0)), verts.getQuick(p.getCoord(1)), verts.getQuick(p.getCoord(2)),
			verts.getQuick(p.getCoord(3)));
		center.scl(1f / 4f);
		...
		for (int j = 0; j < tris.size(); j++) {
			...
			tmp.set(verts.getQuick(t.vmax)).sub(verts.getQuick(t.getCoord(0)));
			t.rise = n.dot(tmp);
		}
		Tri te;
		vlimit -= 4;
		while (vlimit > 0 && ((te = extrudable(epsilon)) != null)) {
			...
			while ((j--) != 0) {
				...
				tmp1.set(verts.getQuick(nt.getCoord(1))).sub(verts.getQuick(nt.getCoord(0)));
				tmp2.set(verts.getQuick(nt.getCoord(2))).sub(verts.getQuick(nt.getCoord(1)));
				tmp.set(tmp1).crs(tmp2);
				if (above(verts, nt, center, 0.01f * epsilon) || tmp.len() < epsilon * epsilon * 0.1f) {
	...
	private Int4 findSimplex (ObjectArrayList<Vector3> verts, int verts_count, IntArray allow, Int4 out) {
		Stack stack = Stack.enter();
		Vector3 tmp = stack.allocVector3();
		Vector3 tmp1 = stack.allocVector3();
		Vector3 tmp2 = stack.allocVector3();

		Vector3[] basis = new Vector3[/* 3 */] {stack.allocVector3(), stack.allocVector3(), stack.allocVector3()};
		basis[0].set(0.01f, 0.02f, 1.0f);
		int p0 = maxdirsterid(verts, verts_count, basis[0], allow);
		tmp.setZero().sub(basis[0]);
		int p1 = maxdirsterid(verts, verts_count, tmp, allow);
		basis[0].set(verts.getQuick(p0)).sub(verts.getQuick(p1));
		...
		tmp.set(1f, 0.02f, 0f);
		basis[1].set(tmp).crs(basis[0]);
		tmp.set(-0.02f, 1f, 0f);
		basis[2].set(tmp).crs(basis[0]);
		if (basis[1].len() > basis[2].len()) {
			basis[1].nor();
		} else {
			basis[1].set(basis[2]);
			basis[1].nor();
		...
		basis[1].set(verts.getQuick(p2)).sub(verts.getQuick(p0));
		basis[2].set(basis[1]).crs(basis[0]);
		basis[2].nor();
		int p3 = maxdirsterid(verts, verts_count, basis[2], allow);
		if (p3 == p0 || p3 == p1 || p3 == p2) {
			tmp.setZero().sub(basis[2]);
		...
		tmp1.set(verts.getQuick(p1)).sub(verts.getQuick(p0));
		tmp2.set(verts.getQuick(p2)).sub(verts.getQuick(p0));
		tmp2.set(tmp1).crs(tmp2);
		tmp1.set(verts.getQuick(p3)).sub(verts.getQuick(p0));
		if (tmp1.dot(tmp2) < 0) {
			int swap_tmp = p2;
			p2 = p3;
			p3 = swap_tmp;
	...
	// BringOutYourDead (John Ratcliff): When you create a convex hull you hand it a large input set of vertices forming a 'point
	// cloud'.
	// After the hull is generated it give you back a set of polygon faces which index the *original* point cloud.
	// The thing is, often times, there are many 'dead vertices' in the point cloud that are on longer referenced by the hull.
	// The routine 'BringOutYourDead' find only the referenced vertices, copies them to an new buffer, and re-indexes the hull so
	// that it is a minimal representation.
	...
	private boolean cleanupVertices (int svcount, ObjectArrayList<Vector3> svertices, int stride, int[] vcount, // output number of
																																					// vertices
		ObjectArrayList<Vector3> vertices, // location to store the results.
		float normalepsilon, Vector3 scale) {
		...
		ObjectArrayList<Vector3> vtx_ptr = svertices;
		int vtx_idx = 0;

		// if ( 1 )
		{
			for (int i = 0; i < svcount; i++) {
				Vector3 p = vtx_ptr.getQuick(vtx_idx);
		...
		for (int i = 0; i < svcount; i++) {
			...
			// if ( 1 )
			{
				...
				if (j == vcount[0]) {
					Vector3 dest = vertices.getQuick(vcount[0]);
					dest.x = px;
					dest.y = py;
					dest.z = pz;
					vcount[0]++;
	...
	private static Vector3 orth (Vector3 v, Vector3 out) {
		Stack stack = Stack.enter();
		Vector3 a = stack.allocVector3();
		a.set(0f, 0f, 1f);
		a.set(v).crs(a);

		Vector3 b = stack.allocVector3();
		b.set(0f, 1f, 0f);
		b.set(v).crs(b);
	...
	private static Vector3 triNormal (Vector3 v0, Vector3 v1, Vector3 v2, Vector3 out) {
		Stack stack = Stack.enter();
		Vector3 tmp1 = stack.allocVector3();
		Vector3 tmp2 = stack.allocVector3();
		...
		tmp1.set(v1).sub(v0);
		tmp2.set(v2).sub(v1);
		Vector3 cp = stack.allocVector3();
		cp.set(tmp1).crs(tmp2);
		float m = cp.len();
		if (m == 0) {
			out.set(1f, 0f, 0f);
			stack.leave();
	...
	private static boolean above (ObjectArrayList<Vector3> vertices, Int3 t, Vector3 p, float epsilon) {
		Stack stack = Stack.enter();
		Vector3 n = triNormal(vertices.getQuick(t.getCoord(0)), vertices.getQuick(t.getCoord(1)), vertices.getQuick(t.getCoord(2)),
			stack.allocVector3());
		Vector3 tmp = stack.allocVector3();
		tmp.set(p).sub(vertices.getQuick(t.getCoord(0)));
	...
	private static void addPoint (int[] vcount, ObjectArrayList<Vector3> p, float x, float y, float z) {
		// XXX, might be broken
		Vector3 dest = p.getQuick(vcount[0]);
		dest.x = x;
		dest.y = y;
		dest.z = z;
		vcount[0]++;
	...
	private static float getDist (float px, float py, float pz, Vector3 p2) {
		float dx = px - p2.x;
		float dy = py - p2.y;
		float dz = pz - p2.z;
