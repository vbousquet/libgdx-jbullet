...
public class PrimitiveTriangle {
	...
	private final ObjectArrayList<Vector3> tmpVecList1 = new ObjectArrayList<Vector3>(TriangleContact.MAX_TRI_CLIPPING);
	private final ObjectArrayList<Vector3> tmpVecList2 = new ObjectArrayList<Vector3>(TriangleContact.MAX_TRI_CLIPPING);
	private final ObjectArrayList<Vector3> tmpVecList3 = new ObjectArrayList<Vector3>(TriangleContact.MAX_TRI_CLIPPING);

	{
		for (int i = 0; i < TriangleContact.MAX_TRI_CLIPPING; i++) {
			tmpVecList1.add(new Vector3());
			tmpVecList2.add(new Vector3());
			tmpVecList3.add(new Vector3());
		}
	}

	public final Vector3[] vertices = new Vector3[3];
	public final Quaternion plane = new Quaternion();
	...
	public void buildTriPlane () {
		...
		tmp1.set(vertices[1]).sub(vertices[0]);
		tmp2.set(vertices[2]).sub(vertices[0]);
	...
	/** Calcs the plane which is paralele to the edge and perpendicular to the triangle plane. This triangle must have its plane
	 * calculated. */
	public void get_edge_plane (int edge_index, Quaternion plane) {
		Stack stack = Stack.enter();
		Vector3 e0 = vertices[edge_index];
		Vector3 e1 = vertices[(edge_index + 1) % 3];
	...
	public int clip_triangle (PrimitiveTriangle other, ObjectArrayList<Vector3> clipped_points) {
		// edge 0
		Stack stack = Stack.enter();
		ObjectArrayList<Vector3> temp_points = tmpVecList1;

		Quaternion edgeplane = stack.allocQuaternion();

		get_edge_plane(0, edgeplane);

		int clipped_count = ClipPolygon.plane_clip_triangle(edgeplane, other.vertices[0], other.vertices[1], other.vertices[2],
			temp_points);

		if (clipped_count == 0) {
			return 0;
		}
		ObjectArrayList<Vector3> temp_points1 = tmpVecList2;
	...
	public boolean find_triangle_collision_clip_method (PrimitiveTriangle other, TriangleContact contacts) {
		...
		ObjectArrayList<Vector3> clipped_points = tmpVecList3;
