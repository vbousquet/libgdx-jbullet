...
public class CylinderShapeZ extends CylinderShape {
	...
	public Vector3 localGetSupportingVertexWithoutMargin(Vector3 vec, Vector3 out) {
		...
		Vector3 result = cylinderLocalSupportZ(getHalfExtentsWithoutMargin(stack.allocVector3()), vec, out);
		stack.leave();
		return result;
	}
	...
	public void batchedUnitVectorGetSupportingVertexWithoutMargin(Vector3[] vectors, Vector3[] supportVerticesOut, int numVectors) {
		...
		for (int i = 0; i < numVectors; i++) {
			cylinderLocalSupportZ(getHalfExtentsWithoutMargin(stack.allocVector3()), vectors[i], supportVerticesOut[i]);
		}
		stack.leave();
	}
