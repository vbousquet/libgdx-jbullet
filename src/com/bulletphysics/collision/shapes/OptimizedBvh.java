...
public class OptimizedBvh implements Serializable {
	...
	public void setInternalNodeAabbMin (int nodeIndex, Vector3 aabbMin) {
		if (useQuantization) {
	...
	public void setInternalNodeAabbMax (int nodeIndex, Vector3 aabbMax) {
		if (useQuantization) {
	...
	public void setQuantizationValues (Vector3 aabbMin, Vector3 aabbMax) {
		setQuantizationValues(aabbMin, aabbMax, 1f);
	}

	public void setQuantizationValues (Vector3 aabbMin, Vector3 aabbMax, float quantizationMargin) {
		...
		bvhAabbMin.set(aabbMin).sub(clampValue);
		bvhAabbMax.set(aabbMax).add(clampValue);
		Vector3 aabbSize = stack.allocVector3();
		aabbSize.set(bvhAabbMax).sub(bvhAabbMin);
	...
	private static class NodeTriangleCallback extends InternalTriangleIndexCallback {
		...
		private final Vector3 aabbMin = new Vector3(), aabbMax = new Vector3();
	...
	public void build (StridingMeshInterface triangles, boolean useQuantizedAabbCompression, Vector3 _aabbMin, Vector3 _aabbMax) {
		...
		if (useQuantization) {
			...
			Vector3 aabbMin = stack.allocVector3();
			aabbMin.set(-1e30f, -1e30f, -1e30f);
	...
	public void refitPartial (StridingMeshInterface meshInterface, Vector3 aabbMin, Vector3 aabbMax) {
...
// }
	...
	public void updateBvhNodes (StridingMeshInterface meshInterface, int firstNode, int endNode, int index) {
		...
		Vector3[] triangleVerts/* [3] */= new Vector3[] {stack.allocVector3(), stack.allocVector3(), stack.allocVector3()};
		Vector3 aabbMin = stack.allocVector3(), aabbMax = stack.allocVector3();
		...
		for (int i = endNode - 1; i >= firstNode; i--) {
			...
			if (curNodes.isLeafNode(curNodeId)) {
				...
				int rightChildNodeId = quantizedContiguousNodes.isLeafNode(leftChildNodeId) ? i + 2 : i + 1
					+ quantizedContiguousNodes.getEscapeIndex(leftChildNodeId);

				for (int i2 = 0; i2 < 3; i2++) {
					curNodes.setQuantizedAabbMin(curNodeId, i2, quantizedContiguousNodes.getQuantizedAabbMin(leftChildNodeId, i2));
					if (curNodes.getQuantizedAabbMin(curNodeId, i2) > quantizedContiguousNodes.getQuantizedAabbMin(rightChildNodeId,
						i2)) {
					...
					if (curNodes.getQuantizedAabbMax(curNodeId, i2) < quantizedContiguousNodes.getQuantizedAabbMax(rightChildNodeId,
						i2)) {
...
// 'startIndex/endIndex'.
	...
	protected void updateSubtreeHeaders (int leftChildNodexIndex, int rightChildNodexIndex) {
		...
		int leftSubTreeSize = quantizedContiguousNodes.isLeafNode(leftChildNodexIndex) ? 1 : quantizedContiguousNodes
			.getEscapeIndex(leftChildNodexIndex);
		int leftSubTreeSizeInBytes = leftSubTreeSize * QuantizedBvhNodes.getNodeSize();

		// btQuantizedBvhNode& rightChildNode = m_quantizedContiguousNodes[rightChildNodexIndex];
		int rightSubTreeSize = quantizedContiguousNodes.isLeafNode(rightChildNodexIndex) ? 1 : quantizedContiguousNodes
			.getEscapeIndex(rightChildNodexIndex);
	...
	protected int sortAndCalcSplittingIndex (int startIndex, int endIndex, int splitAxis) {
		...
		for (i = startIndex; i < endIndex; i++) {
			center.set(getAabbMax(i)).add(getAabbMin(i));
			center.scl(0.5f);
			means.add(center);
		}
...
// break;
			...
			case RECURSIVE:
				walkRecursiveQuantizedTreeAgainstQueryAabb(quantizedContiguousNodes, 0, nodeCallback, quantizedQueryAabbMin,
	...
	protected void walkStacklessTree (NodeOverlapCallback nodeCallback, Vector3 aabbMin, Vector3 aabbMax) {
		assert (!useQuantization);
	...
	protected void walkRecursiveQuantizedTreeAgainstQueryAabb (QuantizedBvhNodes currentNodes, int currentNodeId,
		...
		if (aabbOverlap) {
			if (isLeafNode) {
				...
				walkRecursiveQuantizedTreeAgainstQueryAabb(currentNodes, leftChildNodeId, nodeCallback, quantizedQueryAabbMin,
					quantizedQueryAabbMax);

				int rightChildNodeId = currentNodes.isLeafNode(leftChildNodeId) ? leftChildNodeId + 1 : leftChildNodeId
					+ currentNodes.getEscapeIndex(leftChildNodeId);
				walkRecursiveQuantizedTreeAgainstQueryAabb(currentNodes, rightChildNodeId, nodeCallback, quantizedQueryAabbMin,
					quantizedQueryAabbMax);
			}
		}
	}

	protected void walkStacklessQuantizedTreeAgainstRay (NodeOverlapCallback nodeCallback, Vector3 raySource, Vector3 rayTarget,
		Vector3 aabbMin, Vector3 aabbMax, int startNodeIndex, int endNodeIndex) {
		assert (useQuantization);
		Stack stack = Stack.enter();
		...
		// #define RAYAABB2
		// #ifdef RAYAABB2
		Vector3 rayFrom = stack.alloc(raySource);
		Vector3 rayDirection = stack.allocVector3();
		tmp.set(rayTarget).sub(raySource);
		rayDirection.set(tmp).nor();
...
// boolean sign_z = rayDirection.z < 0f;
		...
		Vector3 rayAabbMin = stack.alloc(raySource);
		Vector3 rayAabbMax = stack.alloc(raySource);
		...
		Vector3 bounds_0 = stack.allocVector3();
		Vector3 bounds_1 = stack.allocVector3();
	...
	public void reportBoxCastOverlappingNodex (NodeOverlapCallback nodeCallback, Vector3 raySource, Vector3 rayTarget,
		...
		if (fast_path) {
			 ...
			 */
			Vector3 qaabbMin = stack.alloc(raySource);
			Vector3 qaabbMax = stack.alloc(raySource);
	...
	public long quantizeWithClamp (Vector3 point) {
		assert (useQuantization);
		Stack stack = Stack.enter();
		...
		v.set(clampedPoint).sub(bvhAabbMin);
	...
	public void unQuantize (Vector3 vecOut, long vecIn) {
