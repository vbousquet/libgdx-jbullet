...
/** PersistentManifold is a contact point cache, it stays persistent as long as objects are overlapping in the broadphase. Those
 * contact points are created by the collision narrow phase.
 * <p>
 * 
 ...
 * <p>
 * 
 * Reduces the cache to 4 points, when more then 4 points are added, using following rules: the contact point with deepest
 * penetration is always kept, and it tries to maximize the area covered by the points.
 * <p>
 * 
...
// #endif //KEEP_DEEPEST_POINT
		...
		if (maxPenetrationIndex != 0) {
			Vector3 a0 = stack.alloc(pt.localPointA);
			a0.sub(pointCache[1].localPointA);

			Vector3 b0 = stack.alloc(pointCache[3].localPointA);
			b0.sub(pointCache[2].localPointA);

			Vector3 cross = stack.allocVector3();
			cross.set(a0).crs(b0);
		...
		if (maxPenetrationIndex != 1) {
			Vector3 a1 = stack.alloc(pt.localPointA);
			a1.sub(pointCache[0].localPointA);

			Vector3 b1 = stack.alloc(pointCache[3].localPointA);
		...
		if (maxPenetrationIndex != 2) {
			Vector3 a2 = stack.alloc(pt.localPointA);
			a2.sub(pointCache[0].localPointA);

			Vector3 b2 = stack.alloc(pointCache[3].localPointA);
		...
		if (maxPenetrationIndex != 3) {
			Vector3 a3 = stack.alloc(pt.localPointA);
			a3.sub(pointCache[0].localPointA);

			Vector3 b3 = stack.alloc(pointCache[2].localPointA);
		...
		Quaternion maxvec = stack.allocQuaternion();
...
// #endif
	...
	public int getCacheEntry (ManifoldPoint newPoint) {
		...
		int nearestPoint = -1;
		Stack stack = Stack.enter();
		Vector3 diffA = stack.allocVector3();
...
// first refresh worldspace positions and distance
		...
		// then
		float distance2d;
		Vector3 projectedDifference = stack.allocVector3(), projectedPoint = stack.allocVector3();
		...
		for (i = getNumContacts() - 1; i >= 0; i--) {
			...
			if (!validContactDistance(manifoldPoint)) {
				...
				tmp.set(manifoldPoint.normalWorldOnB).scl(manifoldPoint.distance1);
