 ...
 * "Ray Casting against bteral Convex Objects with Application to Continuous Collision Detection" GJK based Ray Cast, optimized
 * version Objects should not start in overlap, otherwise results are not defined.
 * 
 * @author jezek2 */
public class SubsimplexConvexCast extends ConvexCast {
	...
	public boolean calcTimeOfImpact (Transform fromA, Transform toA, Transform fromB, Transform toB, CastResult result) {
		...
		Vector3 supVertexA = convexA.localGetSupportingVertex(tmp, stack.allocVector3());
		fromA.transform(supVertexA);

		MatrixUtil.transposeTransform(tmp, r, fromB.basis);
		Vector3 supVertexB = convexB.localGetSupportingVertex(tmp, stack.allocVector3());
		...
		// #endif
		Vector3 w = stack.allocVector3(), p = stack.allocVector3();
		float VdotR;
		...
		if (n.len2() >= BulletGlobals.SIMD_EPSILON * BulletGlobals.SIMD_EPSILON) {
			result.normal.set(n).nor();
		} else {
		...
		Vector3 hitA = stack.allocVector3();
		Vector3 hitB = stack.allocVector3();
