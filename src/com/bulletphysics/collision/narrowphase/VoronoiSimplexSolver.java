...
/** VoronoiSimplexSolver is an implementation of the closest point distance algorithm from a 1-4 points simplex to the origin. Can
 * be used with GJK, as an alternative to Johnson distance algorithm.
 * 
 * @author jezek2 */
public class VoronoiSimplexSolver extends SimplexSolverInterface {
	...
	public boolean updateClosestVectorAndPoints () {
		if (needsUpdate) {
			...
			case 1: {
				...
				cachedV.set(cachedP1).sub(cachedP2); // == m_simplexVectorW[0]
				cachedBC.reset();
			...
			case 2: {
				...
				Vector3 nearest = stack.allocVector3();
			...
			case 3: {
				...
				tmp1.set(simplexPointsP[0]).scl(cachedBC.barycentricCoords[0]);
				tmp2.set(simplexPointsP[1]).scl(cachedBC.barycentricCoords[1]);
				tmp3.set(simplexPointsP[2]).scl(cachedBC.barycentricCoords[2]);
				VectorUtil.add(cachedP1, tmp1, tmp2, tmp3);

				tmp1.set(simplexPointsQ[0]).scl(cachedBC.barycentricCoords[0]);
				tmp2.set(simplexPointsQ[1]).scl(cachedBC.barycentricCoords[1]);
				tmp3.set(simplexPointsQ[2]).scl(cachedBC.barycentricCoords[2]);
			...
			case 4: {
				...
				if (hasSeperation) {
					...
					tmp2.set(simplexPointsP[1]).scl(cachedBC.barycentricCoords[1]);
					tmp3.set(simplexPointsP[2]).scl(cachedBC.barycentricCoords[2]);
					tmp4.set(simplexPointsP[3]).scl(cachedBC.barycentricCoords[3]);
					VectorUtil.add(cachedP1, tmp1, tmp2, tmp3, tmp4);

					tmp1.set(simplexPointsQ[0]).scl(cachedBC.barycentricCoords[0]);
					tmp2.set(simplexPointsQ[1]).scl(cachedBC.barycentricCoords[1]);
					tmp3.set(simplexPointsQ[2]).scl(cachedBC.barycentricCoords[2]);
					tmp4.set(simplexPointsQ[3]).scl(cachedBC.barycentricCoords[3]);
	...
	public boolean closestPtPointTriangle (Vector3 p, Vector3 a, Vector3 b, Vector3 c, SubSimplexClosestResult result) {
		...
		Vector3 ac = stack.allocVector3();
		ac.set(c).sub(a);

		Vector3 ap = stack.allocVector3();
		ap.set(p).sub(a);
		...
		if (vc <= 0f && d1 >= 0f && d3 <= 0f) {
			float v = d1 / (d1 - d3);
			result.closestPointOnSimplex.x = v * ab.x + a.x;
			result.closestPointOnSimplex.y = v * ab.y + a.y;
			result.closestPointOnSimplex.z = v * ab.z + a.z;
			result.usedVertices.usedVertexA = true;
			result.usedVertices.usedVertexB = true;
		...
		if (vb <= 0f && d2 >= 0f && d6 <= 0f) {
			float w = d2 / (d2 - d6);
			result.closestPointOnSimplex.x = w * ac.x + a.x;
			result.closestPointOnSimplex.y = w * ac.y + a.y;
			result.closestPointOnSimplex.z = w * ac.z + a.z;
			result.usedVertices.usedVertexA = true;
			result.usedVertices.usedVertexC = true;
		...
		if (va <= 0f && (d4 - d3) >= 0f && (d5 - d6) >= 0f) {
			...
			Vector3 tmp = stack.allocVector3();
			tmp.set(c).sub(b);
			result.closestPointOnSimplex.x = w * tmp.x + b.x;
			result.closestPointOnSimplex.y = w * tmp.y + b.y;
			result.closestPointOnSimplex.z = w * tmp.z + b.z;
...
// #else
		...
		try {
			...
			if (pointOutsideABC != 0) {
				...
				if (sqDist < bestSqDist) {
					...
					finalResult.setBarycentricCoordinates(tempResult.barycentricCoords[VERTA], tempResult.barycentricCoords[VERTB],
						tempResult.barycentricCoords[VERTC], 0);
			...
			if (pointOutsideACD != 0) {
				...
				if (sqDist < bestSqDist) {
					...
					finalResult.setBarycentricCoordinates(tempResult.barycentricCoords[VERTA], 0, tempResult.barycentricCoords[VERTB],
			...
			if (pointOutsideADB != 0) {
				...
				if (sqDist < bestSqDist) {
					...
					finalResult.setBarycentricCoordinates(tempResult.barycentricCoords[VERTA], tempResult.barycentricCoords[VERTC], 0,
			...
			if (pointOutsideBDC != 0) {
				...
				if (sqDist < bestSqDist) {
					...
					finalResult.setBarycentricCoordinates(0, tempResult.barycentricCoords[VERTA], tempResult.barycentricCoords[VERTC],
						tempResult.barycentricCoords[VERTB]);
	...
	public float maxVertex () {
		...
		for (i = 0; i < numverts; i++) {
			float curLen2 = simplexVectorW[i].len2();
			if (maxV < curLen2) {
				maxV = curLen2;
			}
		}
