...
public class GjkPairDetector extends DiscreteCollisionDetectorInterface {
	...
	public void getClosestPoints (ClosestPointInput input, Result output, IDebugDraw debugDraw, boolean swapResults) {
		...
		float distance = 0f;
		Vector3 normalInB = stack.allocVector3();
		normalInB.set(0f, 0f, 0f);
		Vector3 pointOnA = stack.allocVector3(), pointOnB = stack.allocVector3();
		Transform localTransA = stack.alloc(input.transformA);
		Transform localTransB = stack.alloc(input.transformB);
		Vector3 positionOffset = stack.allocVector3();
		positionOffset.set(localTransA.origin).add(localTransB.origin);
		positionOffset.scl(0.5f);
...

		{
			...
			Vector3 seperatingAxisInA = stack.allocVector3();
			Vector3 seperatingAxisInB = stack.allocVector3();

			Vector3 pInA = stack.allocVector3();
			Vector3 qInB = stack.allocVector3();

			Vector3 pWorld = stack.allocVector3();
			Vector3 qWorld = stack.allocVector3();
			Vector3 w = stack.allocVector3();

			Vector3 tmpPointOnA = stack.allocVector3(), tmpPointOnB = stack.allocVector3();
			Vector3 tmpNormalInB = stack.allocVector3();

			for (;;) // while (true)
			{
				seperatingAxisInA.set(cachedSeparatingAxis).scl(-1);
				...
				if (cachedSeparatingAxis.len2() < REL_ERROR2) {
					degenerateSimplex = 6;
					checkSimplex = true;
					break;
				}
				...
				squaredDistance = cachedSeparatingAxis.len2();
			...
			if (checkSimplex) {
				...
				float lenSqr = cachedSeparatingAxis.len2();
				// valid normal
				if (lenSqr < 0.0001f) {
					degenerateSimplex = 5;
				}
				if (lenSqr > BulletGlobals.FLT_EPSILON * BulletGlobals.FLT_EPSILON) {
					...
					normalInB.scl(rlen); // normalize
					float s = (float)Math.sqrt(squaredDistance);

					assert (s > 0f);

					tmp.set(cachedSeparatingAxis).scl((marginA / s));
					pointOnA.sub(tmp);

					tmp.set(cachedSeparatingAxis).scl((marginB / s));
					pointOnB.add(tmp);
			...
			if (checkPenetration && (!isValid || catchDegeneratePenetrationCase)) {
				...
				if (penetrationDepthSolver != null) {
					...
					boolean isValid2 = penetrationDepthSolver.calcPenDepth(simplexSolver, minkowskiA, minkowskiB, localTransA,
						localTransB, cachedSeparatingAxis, tmpPointOnA, tmpPointOnB, debugDraw/* ,input.stackAlloc */);

					if (isValid2) {
						tmpNormalInB.set(tmpPointOnB).sub(tmpPointOnA);

						float lenSqr = tmpNormalInB.len2();
						if (lenSqr > (BulletGlobals.FLT_EPSILON * BulletGlobals.FLT_EPSILON)) {
							tmpNormalInB.scl(1f / (float)Math.sqrt(lenSqr));
							tmp.set(tmpPointOnA).sub(tmpPointOnB);
							float distance2 = -tmp.len();
