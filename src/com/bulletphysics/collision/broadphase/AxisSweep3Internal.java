...
/** AxisSweep3Internal is an internal base class that implements sweep and prune. Use concrete implementation {@link AxisSweep3}
 * or {@link AxisSweep3_32}.
 * 
 * @author jezek2 */
public abstract class AxisSweep3Internal extends BroadphaseInterface {
	...
	protected final Vector3 worldAabbMin = new Vector3(); // overall system bounds
	protected final Vector3 worldAabbMax = new Vector3(); // overall system bounds

	protected final Vector3 quantize = new Vector3(); // scaling factor for quantization
	...
	protected EdgeArray[] pEdges = new EdgeArray[3]; // edge arrays for the 3 axes (each array has m_maxHandles * 2 + 2 sentinel
																		// entries)

	protected OverlappingPairCache pairCache;

	// OverlappingPairCallback is an additional optional user callback for adding/removing overlapping pairs, similar interface to
	...
	AxisSweep3Internal (Vector3 worldAabbMin, Vector3 worldAabbMax, int handleMask, int handleSentinel,
	...
	protected void quantize (int[] out, Vector3 point, int isMax) {
		Stack stack = Stack.enter();
		...
		v.set(clampedPoint).sub(worldAabbMin);
	...
	public int addHandle (Vector3 aabbMin, Vector3 aabbMax, Object pOwner, short collisionFilterGroup, short collisionFilterMask,
	...
	public void updateHandle (int handle, Vector3 aabbMin, Vector3 aabbMax, Dispatcher dispatcher) {
		Handle pHandle = getHandle(handle);
	...
	public BroadphaseProxy createProxy (Vector3 aabbMin, Vector3 aabbMax, BroadphaseNativeType shapeType, Object userPtr,
