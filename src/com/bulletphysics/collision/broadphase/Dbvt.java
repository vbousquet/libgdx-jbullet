...
public class Dbvt {
	...
	public boolean update (Node leaf, DbvtAabbMm volume, Vector3 velocity) {
		if (leaf.volume.Contain(volume)) {
			return false;
		}
		volume.SignedExpand(velocity);
	...
	public static void collideRAY (Node root, Vector3 origin, Vector3 direction, ICollide policy) {
		// DBVT_CHECKTYPE
		if (root != null) {
			Stack st = Stack.enter();
			Vector3 normal = st.allocVector3();
			normal.set(direction).nor();
			Vector3 invdir = st.allocVector3();
	...
	public static void collideKDOP (Node root, Vector3[] normals, float[] offsets, int count, ICollide policy) {
		// DBVT_CHECKTYPE
		if (root != null) {
	...
	public static void collideOCL (Node root, Vector3[] normals, float[] offsets, Vector3 sortaxis, int count, ICollide policy) {
	...
	public static void collideOCL (Node root, Vector3[] normals, float[] offsets, Vector3 sortaxis, int count, ICollide policy,
		...
		if (root != null) {
			...
			do {
				// JAVA NOTE: check
				int id = stack.removeIndex(stack.size - 1);
	...
	private static float size (DbvtAabbMm a) {
		Stack stack = Stack.enter();
		Vector3 edges = a.Lengths(stack.allocVector3());
	...
	private static void insertleaf (Dbvt pdbvt, Node root, Node leaf) {
		if (pdbvt.root == null) {
			...
			if (!root.isleaf()) {
				do {
					if (DbvtAabbMm.Proximity(root.childs[0].volume, leaf.volume) < DbvtAabbMm.Proximity(root.childs[1].volume,
						leaf.volume)) {
						root = root.childs[0];
					} else {
						root = root.childs[1];
	...
	private static Vector3[] axis = new Vector3[] {new Vector3(1, 0, 0), new Vector3(0, 1, 0), new Vector3(0, 0, 1)};
	...
	private static Node topdown (Dbvt pdbvt, ObjectArrayList<Node> leaves, int bu_treshold) {
		if (leaves.size() > 1) {
			if (leaves.size() > bu_treshold) {
				...
				Vector3 org = vol.Center(stack.allocVector3());
