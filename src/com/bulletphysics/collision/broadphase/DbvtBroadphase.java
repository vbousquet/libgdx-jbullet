...
public class DbvtBroadphase extends BroadphaseInterface {
	...
	public void setAabb (BroadphaseProxy absproxy, Vector3 aabbMin, Vector3 aabbMax, Dispatcher dispatcher) {
		DbvtProxy proxy = (DbvtProxy)absproxy;
		...
		if (proxy.stage == STAGECOUNT) {
			...
			if (DbvtAabbMm.Intersect(proxy.leaf.volume, aabb)) {/* Moving */
				...
				Vector3 delta = stack.allocVector3();
				delta.set(aabbMin).add(aabbMax);
				delta.scl(0.5f);
				delta.sub(proxy.aabb.Center(stack.allocVector3()));
				// #ifdef DBVT_BP_MARGIN
				delta.scl(predictedframes);
	...
	public void getBroadphaseAabb (Vector3 aabbMin, Vector3 aabbMax) {
		...
		if (!sets[0].empty()) {
			...
			DbvtAabbMm.FromCR(new Vector3(0f, 0f, 0f), 0f, bounds);
		}
