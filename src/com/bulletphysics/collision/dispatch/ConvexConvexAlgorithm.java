...
// #else
	...
	public float calculateTimeOfImpact (CollisionObject col0, CollisionObject col1, DispatcherInfo dispatchInfo,
		...
		tmp.set(col0.getInterpolationWorldTransform(tmpTrans1).origin).sub(col0.getWorldTransform(tmpTrans2).origin);
		float squareMot0 = tmp.len2();

		tmp.set(col1.getInterpolationWorldTransform(tmpTrans1).origin).sub(col1.getWorldTransform(tmpTrans2).origin);
		float squareMot1 = tmp.len2();
		...
		// also the mainloop of the physics should have a kind of toi queue (something like Brian Mirtich's application of Timewarp
		// for Rigidbodies)
		...
		// Convex0 against sphere for Convex1
		{
			...
			SphereShape sphere1 = new SphereShape(col1.getCcdSweptSphereRadius()); // todo: allow non-zero sphere sizes, for better
																											// approximation
		...
		// Sphere (for convex0) against Convex1
		{
			...
			SphereShape sphere0 = new SphereShape(col0.getCcdSweptSphereRadius()); // todo: allow non-zero sphere sizes, for better
																											// approximation
			ConvexCast.CastResult result = new ConvexCast.CastResult();
			VoronoiSimplexSolver voronoiSimplex = new VoronoiSimplexSolver();
			// SubsimplexConvexCast ccd0(&sphere,min0,&voronoiSimplex);
